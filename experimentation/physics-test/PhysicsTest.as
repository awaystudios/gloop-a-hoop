package
{
	...
	{
		[Embed(source="../assets/testmodels/torus2.obj", mimeType="application/octet-stream")]
...
//			_cameraController.inputContext = new DefaultMouseKeyboardInputContext( this, stage );
			...
			_ballMaterial.lightPicker = lights;
			_targetMaterial = new ColorMaterial( 0x0000FF );
			_targetMaterial.lightPicker = lights;
			_transparentWallMaterial = new ColorMaterial( 0x666666, 0.5 );
			_transparentWallMaterial.lightPicker = lights;
...
//			_view.scene.addChild( trident );
		...
		private function initLevel():void {
			// ground ---------------
			var groundMesh:Mesh = new Mesh( new PlaneGeometry( 50000, 50000 ), _wallMaterial );
			...
			// back wall
			var geometry:CubeGeometry = new CubeGeometry( _roomDimensions.x, _roomDimensions.y, _wallThickness );
			var shape:AWPBoxShape = new AWPBoxShape( geometry.width, geometry.height, geometry.depth );
			var position:Vector3D = new Vector3D( 0, groundBody.y + geometry.height / 2, _roomDimensions.z );
			createBox( geometry, shape, position, _wallMaterial );
			// right wall
			geometry = new CubeGeometry( _wallThickness, _roomDimensions.y, _roomDimensions.z );
			shape = new AWPBoxShape( geometry.width, geometry.height, geometry.depth );
			position = new Vector3D( _roomDimensions.x / 2, groundBody.y + geometry.height / 2, _roomDimensions.z / 2 );
			createBox( geometry, shape, position, _wallMaterial );
			// left wall
			position = new Vector3D( -_roomDimensions.x / 2, groundBody.y + geometry.height / 2, _roomDimensions.z / 2 );
			createBox( geometry, shape, position, _wallMaterial );
...
//			createBox( dimensions, position, _wallMaterial );
			// fixed obstacles ---------------
//			geometry = new CubeGeometry( _roomDimensions.x / 4, _roomDimensions.y, _wallThickness );
//			position = new Vector3D( 100, groundBody.y + _roomDimensions.y / 2, 0.25 * _roomDimensions.z );
//			var box:AWPRigidBody = createBox( geometry, shape, position, _transparentWallMaterial );
//			box.rotation = new Vector3D( 0, 30, 0 );
			// free obstacles --------------
			resetObstacles();
			// target ---------------
			geometry = new CubeGeometry( 800, 800, 800 );
			position = new Vector3D( _roomDimensions.x / 4, groundBody.y + _roomDimensions.y / 2, _roomDimensions.z - 100 );
			_targetBody = createBox( geometry, shape, position, _targetMaterial );
		}
...
//			createBoxGrid( 4, 6, 450, 0.75 * _roomDimensions.y, 2500, new Vector3D( 750, _groundY, 0.35 * _roomDimensions.z ), 0.5 );
		...
		private function createBox( geometry:CubeGeometry, shape:AWPBoxShape, position:Vector3D, material:ColorMaterial, mass:Number = 0, damping:Number = 1 ):AWPRigidBody {
		...
		private function initGloop():void {
			...
			_ballMesh = new Mesh( new SphereGeometry( 50 ), _ballMaterial );
			_ballMesh.mouseEnabled = true;
			_ballMesh.mouseHitMethod = MouseHitMethod.BOUNDS_ONLY;
